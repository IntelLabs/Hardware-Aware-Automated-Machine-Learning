...
def mask_ssm(layers, layer_indices):
    for idx in range(len(layers)):
        assert hasattr(layers[idx], "mixer") and hasattr(layers[idx].mixer, "mask_ssm")
        if idx in layer_indices:
            layers[idx].mixer.mask_ssm = True
        else:
            layers[idx].mixer.mask_ssm = False
...
def main():
    ...
    if do_prune:
        ...
        while True:
            ...
            for target_id in tqdm(candidate_targets):
                ...
                mask_func(model.backbone.layers, pruning_config)
    ...
    if do_eval:
        ...
        if "pruned_mamba_block_idx" in pruned_config:
            mask_mamba_block(model.backbone.layers, pruned_config["pruned_mamba_block_idx"])
        if "pruned_ssm_idx" in pruned_config:
            mask_ssm(model.backbone.layers, pruned_config["pruned_ssm_idx"])

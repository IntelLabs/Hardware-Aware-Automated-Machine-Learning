...
# input_csv="experiments_performance_improvement_with_kv_sparsity.csv"
# input_csv="experiments.csv"
input_csv="experiments_attention.csv"
...
num_repeats=1
use_cached_prefill=True
# use_cached_prefill=False
# Read the header and write it to the output CSV
header=$(head -n 1 "$input_csv")
echo "$header" > "$output_csv"

# Process the CSV file line by line, skipping the header
tail -n +2 "$input_csv" | tr -d '\r' | while IFS=, read -r model_id saved_model_path context_length num_generated_tokens mode num_groups num_threads core batch_size enable_custom_attention use_custom_k use_custom_v k_pruning_percentage v_pruning_percentage int8 attention_mode; do
  ...
  for ((i=1; i<=$num_repeats; i++)); do
    ...
    int8_str=""
    if [ "$int8" = "True" ]; then
        int8_str="--enable_int8"
    fi

    use_cached_prefill_str=""
    if [ "$use_cached_prefill" = "True" ]; then
        use_cached_prefill_str="--use_cached_prefill"
    fi

    # Run the experiment
    numactl --cpunodebind 0 --membind 0 --physcpubind=$core_vals python llm_pipeline.py --model_id $model_id --saved_model_path $saved_model_path --context_length $context_length --num_generated_tokens $num_generated_tokens --mode $mode --num_threads $num_threads --batch_size $batch_size $enable_custom_attention_str $use_custom_k_str $use_custom_v_str --k_pruning $k_pruning_percentage --v_pruning $v_pruning_percentage --attention_mode $attention_mode $int8_str $use_cached_prefill_str
    ...
    new_line="$model_id,$saved_model_path,$context_length,$num_generated_tokens,$mode,$num_groups,$num_threads,$core,$batch_size,$enable_custom_attention,$use_custom_k,$use_custom_v,$k_pruning_percentage,$v_pruning_percentage,$int8,$attention_mode,$time_prefill,$time_decode,$total_time"

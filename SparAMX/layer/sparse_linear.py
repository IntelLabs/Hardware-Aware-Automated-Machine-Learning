...
from sparamx import sparse_linear
import ctypes
...
class SparseLinear(nn.Linear):
    ...
    def load_from_file(cls, path):
        ...
        # Make sure weight_metadata is cache aligned
        if new_inst.weight_metadata.data_ptr() % 64 != 0:  # Use data_ptr() instead of ctypes.addressof()
            new_inst.weight_metadata = nn.Parameter(new_inst.weight_metadata.data.clone(), requires_grad=False)
    ...
    def split_weights_to_sets_batched(self, num_sets):
        ...
        for i in range(self.weight_metadata.shape[0]):
            for j in range(self.weight_metadata.shape[1]):
                assert self.weight_metadata[i][j].numel() % num_sets == 0, "Number of sets doesn't divide weights equally."
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        output = torch.zeros(input.shape[0], input.shape[1], self.out_cols, dtype=torch.float32, device=input.device).contiguous()
    ...
    def from_batched_weights(cls, orig_weights, num_threads=32, shallow=False):
        ...
        if weights.shape[3] > 32:
            ...
            tile_width = 32
            tile_height = 16
            if weights.shape[3] // num_threads > tile_width:
                grouped_col_tiles = True
            else:
                grouped_col_tiles = False
            weights = weights.view(weights.shape[0], weights.shape[1], weights.shape[2], -1, tile_width).transpose(2, 3)
            if grouped_col_tiles:
                weights = weights.view(weights.shape[0], weights.shape[1], weights.shape[2], weights.shape[3] // tile_height, tile_height, weights.shape[4]).transpose(2,3)
                weights = weights.view(weights.shape[0], weights.shape[1], weights.shape[2], weights.shape[3] // 2, 2, weights.shape[4], weights.shape[5]).transpose(2,3)

        # import pdb; pdb.set_trace()
        # new_inst.populate_sparse_weights_batched(weights.reshape(orig_weights.shape[0], orig_weights.shape[1], orig_weights.shape[2], -1).contiguous())

# Credits: https://github.com/BayesWatch/nas-without-training/blob/master/nasspace.py
import itertools
import random
import numpy as np
    ...
    def get_accuracy(self, uid, acc_type, trainval=True):
        try:
            if self.dataset == 'cifar10':
                acc_results = sum([self.api.get_more_info(uid, 'cifar10-valid', None,
                                                        use_12epochs_result=False,
                                                        is_random=seed)['valid-accuracy'] for seed in [777, 888, 999]])/3.
                val_acc = acc_results['valid-accuracy']
            else:
                acc_results = sum([self.api.get_more_info(uid, self.dataset, None,
                                                        use_12epochs_result=False,
                                                        is_random=seed)['valid-accuracy'] for seed in [777, 888, 999]])/3.
                val_acc = acc_results['valid-accuracy']
            return val_acc
            ...
            if self.dataset == 'cifar10':
                acc_results = self.api.get_more_info(arch_index, 'cifar10-valid', None,
                                                        use_12epochs_result=False,
                                                        is_random=False)['valid-accuracy'] 
                val_acc = acc_results['valid-accuracy']
            else:
                acc_results = self.api.get_more_info(arch_index, self.dataset, None,
                                                        use_12epochs_result=False,
                                                        is_random=False)['valid-accuracy'] 
                val_acc = acc_results['valid-accuracy']
            return val_acc

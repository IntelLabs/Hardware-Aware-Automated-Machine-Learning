...
EVOLUTION_DATASET = ['net_stat_NN_22_stats_cifar10_NASBench201__1000.pt', 
...
TEST_DATASET = ['net_stat_NN_22_stats_cifar100_NASBench201__1000.pt', 
            ...
            'net_stat_NN_22_stats_ImageNet_NDS_nds_pnas_in_20.pt', 'net_stat_NN_22_stats_ImageNet_NDS_nds_amoeba_in_20.pt', 'net_stat_NN_22_stats_ImageNet_NDS_nds_darts_in_20.pt']
...
dataset_files_available = os.listdir('/data2/yakhauri/datasets/')
...
for item in EVOLUTION_DATASET:
    for file_name in dataset_files_available:
        if file_name.__contains__(item):
            evolution_filename_list.append('/data2/yakhauri/datasets/' + file_name)
...
for item in TEST_DATASET:
    for file_name in dataset_files_available:
        if file_name.__contains__(item):
            test_filename_list.append('/data2/yakhauri/datasets/' + file_name)
# test_choice_dict = {}
...
def evalOneMaxMultiSet(individual):
    ...
    for item in net_score_superset:
        ...
        if np.isfinite(np.asarray(nets_processed_score)).all()==False:
            return -1, -1
        metric_scores_tau = []
        metric_scores_perason = []
        for idx in range(len(nets_processed_score)):
            # if METRIC=='kendalltau':
            metric_tau, p_score = stats.kendalltau(np.asarray(nets_processed_score[idx]), np.asarray(net_test_acc_superset[idx]))
            # else:
            metric_pearson, _ = stats.pearsonr(np.asarray(nets_processed_score[idx]), np.asarray(net_test_acc_superset[idx]))
            if math.isnan(metric_tau) or math.isinf(metric_tau):
                return -1, -1
            if math.isnan(metric_pearson) or math.isinf(metric_pearson):
                return -1, -1
            metric_scores_tau.append(metric_tau)
            metric_scores_perason.append(metric_pearson)
        metric_tau_min = min([x  if (x==-1 or x>0) else -1*x for x in metric_scores_tau])
        metric_pearson_min = min([x  if (x==-1 or x>0) else -1*x for x in metric_scores_perason])
        # min_tau = 
        return metric_tau_min, metric_pearson_min
...
if __name__=="__main__":
    ...
    for dkey in EVOLUTION_DATASET:
        ...
        print("Datset: ", dkey)
        current_evol_full_dict = torch.load('/data2/yakhauri/datasets/'+dkey)
    ...
    for dkey in TEST_DATASET:
        ...
        print("Datset: ", dkey)
        current_test_full_dict = torch.load('/data2/yakhauri/datasets/'+dkey)
        print("Dataset loading time: ", time.time() - st_l)

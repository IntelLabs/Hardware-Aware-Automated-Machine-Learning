...
dataset_path = '/data2/yakhauri/datasets_trained/net_stat_NN_22_stats_' + dataset + "_" + search_space + "_" + nds_space + "_" + str(NUM_NETS) + ".pt"
...
if dataset=='cifar100':
    ...
    train_loader_net_training = get_cifar100_train_loader(batch_size=training_batch_size)
elif dataset=='cifar10':
    train_loader = get_train_loader(batch_size=batch_size)
    train_loader_net_training = get_train_loader(batch_size=training_batch_size)
elif dataset=='ImageNet16-120':
    train_loader = get_imagenet_train_loader(batch_size=batch_size)
    train_loader_net_training = get_imagenet_train_loader(batch_size=training_batch_size)
elif dataset=='ImageNet':
    train_loader = get_imagenet_224_train_loader(batch_size=batch_size)
    train_loader_net_training = get_imagenet_224_train_loader(batch_size=training_batch_size)
...
while i_reg  < NUM_NETS:
    try:
        ...
        for _ in range(num_epoch_train):
            ...
            correct = 0
            total = 0
            for batch_idx, (tt_inps, tt_tar) in enumerate(train_loader):
                tt_inps, tt_tar = tt_inps.cuda(), tt_tar.cuda()
                optimizer.zero_grad()
                outputs = net(tt_inps)
                loss = criterion(outputs, tt_tar)
                loss.backward()
                optimizer.step()
                train_loss += loss.item()
                _, predicted = outputs.max(1)
                total += tt_tar.size(0)
                correct += predicted.eq(tt_tar).sum().item()
                print("Train Loss: ", str(train_loss)[:6], " Accuracy: ", str(100.*correct/total)[:6])        
            # scheduler.step()

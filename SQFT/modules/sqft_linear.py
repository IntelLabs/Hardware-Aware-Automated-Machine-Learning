...
class SQFTQuantAwareLinear(QuantLinear):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.training:
            ...
            # This forward pass is not the correct one; it is only used for constructing the NNCF graph.
            # This class is intended for quantization-aware training only.
            # Please merge the adapter with the base model before performing the correct forward inference.
            result = self.quant_linear_module(x) + lora_B(
                lora_A(dropout(x.to(lora_A.weight.dtype)))
            ) * scaling
            result = result.to(result_dtype)

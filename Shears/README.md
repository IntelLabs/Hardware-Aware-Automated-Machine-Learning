...
Note that we can also use the aforementioned command to directly test our released models. Simply pass `${SHEARS_PATH}/base_model` to `--model_name_or_path`, `${SHEARS_PATH}/adapter_model` to `--lora_weights`, and the corresponding NNCF config to `--nncf_config` (see [Table](#released-models-)).

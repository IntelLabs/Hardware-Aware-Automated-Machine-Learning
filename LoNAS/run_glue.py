#!/usr/bin/env python
# coding=utf-8
# Based on https://github.com/huggingface/transformers/blob/v4.31.0/examples/pytorch/text-classification/run_glue.py
...
class LonasTrainingArguments(TrainingArguments):
    ...
    target_modules: str = field(
        default="query,key,value", metadata={"help": "Which module will be added the lora adapter."}
    )
...
def main():
    ...
    if training_args.nncf_config is not None:
        ...
        if training_args.do_train:
            train_dataloader = get_train_dataloader_for_init(
                training_args, train_dataset, data_collator=default_data_collator
            )
            ...
            initializing_data_loader_cls = PTInitializingDataLoader
            ...
            nncf_config.register_extra_structs(
                [QuantizationRangeInitArgs(initializing_data_loader), BNAdaptationInitArgs(initializing_data_loader)]
            )
    ...
    if training_args.to_onnx:
        ...
        if nncf_config is not None:
            ...
            onnx.export(model, (dummy_tensor, dummy_tensor, dummy_tensor), training_args.to_onnx, opset_version=10)
    ...
    if training_args.do_search and nncf_config is not None:
        ...
        elasticity_ctrl, best_config, performance_metrics = search_algo.run(
            validate_model_fn, eval_loader, training_args.output_dir
        )
